...
<div class="loginWrapper">
    <div class="logIn">
        <div class="loginInner">
            <a-row v-if="record">
                <a-col :sm="24" :xs="24">
                    <div class="rightWrapper">
                        ...
                        <a-form ref="formRef" :model="escalationAction" layout="vertical" autocomplete="off"  @finish="submitForm">
                            <a-row>
                            ...
                            <a-col :sm="24" :xs="24">
                                <div class="escalationForm">
                                    <div class="patientDetails">
                                        ...
                                        <div class="info">
                                            <h3><a @click="showPatientModal(patient.patientDetails?.id)">{{patient.patientDetails?.fullName}}</a></h3>
                                            <p> <b>DOB : </b> <span>{{patient.patientDetails?.patientDob}}</span></p>
                                            <p v-if="patient.patientDetails?.genderName"><b>Gender :</b> {{patient.patientDetails?.genderName}}</p>
                                            ...
                                            <p><b>Escalation Type : </b><a-tag :color="record?.color">{{record?.type}}</a-tag> </p>
                                        
                                        </div>
                                        <div class="referalPhysicsan" >
                                            <h3>Physician</h3>
                                            <p><b> Name : </b> <a @click="showStaffModal(userName?.user?.staffUdid)">{{userName?.user?.name}}</a></p>
                            ...
                            <a-col :sm="24" :xs="24">
                                <h3>Reason for Escalation:</h3><p>{{record?.reason}}</p>
                            </a-col>
                            
                            <a-col :sm="24" :xs="24">
                                <a-col :span="24">
                                    <h3 class="graphDetails">Latest Reading</h3>
                                </a-col>
                                <a-col class="mb-10" :sm="24" v-if="!patient.devices?.length>0">
                            ...
                            <a-col :sm="24" :xs="24">
                                <a-tabs v-model:activeKey="activeKey" type="card">
                                  <a-tab-pane key="1" tab="Vitals" >
                                    ...
                                    <PatientVitalsGrid :isEscalationAction="true" :patientId="patient.patientDetails?.id" v-if="patient.devices?.length>0"/>
                                  </a-tab-pane>
                                  <a-tab-pane key="2" tab="Notes" v-if="escalation?.escalationNots?.length>0">
                                    <a-table :scroll="{ x: 900 }"   class="sortIconTable" rowKey="id" :columns="escalation.notesColumns" :data-source="escalation.escalationNots" :pagination="false">
                                  ...
                                  <a-tab-pane key="3" tab="Flags" v-if="escalation?.escalationFlags?.length>0">
                                    <a-table :scroll="{ x: 900 }"  class="sortIconTable" rowKey="id" :columns="escalation.flagColumns" :data-source="escalation.escalationFlags" :pagination="false">
                                  ...
                                  <a-tab-pane key="4" tab="Care Plans" v-if="escalation?.escalationCarePlan?.length>0">
                                    <a-table :scroll="{ x: 900 }"  class="sortIconTable" rowKey="id" :columns="escalation.carePlanColumns" :data-source="escalation.escalationCarePlan" :pagination="false">
                                        </a-table>
                                  </a-tab-pane>
                                  
                                  <a-tab-pane key="5" tab="Escalation" v-if="escalation?.escalationList?.length>0">
                                    <a-table :scroll="{ x: 1200 }"  class="sortIconTable" rowKey="id" :columns="escalation.columnData" :data-source="escalation.escalationList" :pagination="false">
                                        <!-- <template #patientName="{ record }" v-if="arrayToObjact(screensPermissions, 405)">
                                             <a @click="showPatientModal(record.patient?.id)" class="nameInfoIcon" v-if="record.patient?.id ">
                                        ...
                                        <template #patientName="{ record }" >
                                             <a @click="showPatientModal(record.patient?.id)" class="nameInfoIcon" v-if="record.patient?.id ">
                                                ...
                                                <!-- <template #assignedBy="{ record }" v-if="arrayToObjact(screensPermissions, 408)">
                                                    <a v-if="record.assignedBy" @click="showStaffModal(record.assignedBy.id)" class="nameInfoIcon">
                                                ...
                                                <!-- <template #escalationAssignedTo="{ record }" v-if="arrayToObjact(screensPermissions, 408)">
                                                    <a v-if="record.escalationAssignTo?.length>0" @click="showDetails(record.escalationAssignTo)">{{record.escalationAssignTo[0]?.staff.fullName.substring(0,10)+'...'}}</a>
                                                </template> -->
                            ...
                            <a-row v-if="record?.status!='Responded'">
                                <a-col :span="24">
                                        <h3 class="mb-10 mt-28">Action Required</h3>
                                </a-col>
                              <a-col :sm="24" :xs="24" :xl="24">
                                    <div class="form-group">
                                        <a-form-item label="Choose Action" name="action" :rules="[{ required: true, message: 'Action'+' '+$t('global.validation') }]">
                                            <GlobalCodeDropDown @change="checkChangeInput()" v-model:value="escalationAction.action"  :dataId="76" @handleGlobalChange="handleGlobalChange($event)" style="width: 100%;"/>
                                        </a-form-item>
                                    </div>
                                </a-col>
                                ...
                                <a-col :sm="24" :xm="24" v-if="escalationAction.action">
                                    <div class="form-group">
                                        <a-form-item label="Note" name="note" :rules="[{ required: true, message: $t('tasks.tasksModal.longDescription')+' '+$t('global.validation')  }]">
                                            <a-textarea v-model:value="escalationAction.note" placeholder="Description" :auto-size="{ minRows: 3 }" @change="checkChangeInput()" />
                                        </a-form-item>
                                    </div>
                                </a-col>
            ...
            <a-row v-else>
                <Login v-if="passwordModal" v-model:visible="passwordModal" @closeModal="closeModal($event)"/>
            </a-row>
        </div>
    </div>
    <Loader />
...
<script>
    ...
    export default defineComponent({
        components: {
            PatientVitalsGrid: defineAsyncComponent(() =>
                 import("@/components/patients/patientSummary/views/PatientVitalsGrid")
            ),
            Flags,
            GlobalCodeDropDown,
            Loader,
            ...
            Login:defineAsyncComponent(() =>
                 import("@/components/login/LoginModal.vue")
            ),
        },
        setup(props,{emit}) {
            ...
            const submitForm = () => {
                ...
                store.dispatch("submitActionForm",{
                    ...
                    if(resp == true){
                        store.dispatch("singleEscalationRecord", route.params.udid)
